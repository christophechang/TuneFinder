...
[![CI](https://github.com/christophechang/TuneFinder/actions/workflows/ci.yml/badge.svg)](https://github.com/christophechang/TuneFinder/actions/workflows/ci.yml)
...
| Volumo | REST API (`/api/v1/albums`) | ✅ (no widget embed — audition page rows are link-only; the SPA plays the prelisten stream inline) |
...
- **Free-download lane** — `pipeline.free_download_sources` (default `[soundcloud]`) routes those sources to the exclusive "Free Downloads" section; `free_downloads_count` (default `5`) and `mix_prep_free_downloads_count` (default `10`) size it per report kind; `free_downloads_min_score` (default `0.0`) is the lane's own floor (the main `section_min_score` deliberately doesn't apply — it's calibrated to store signals the lane can't have)
